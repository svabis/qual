...
#            d = user_data[2]
            ...
            mailbox_data = check_mail(d) # call method to dowlaod e-mails [0]-datetime newest mail | [1]-Update (True/False)
#            print mailbox_data

            if mailbox_data[0] != None:
                temp = mailbox_data[0]
                if d[1] != temp: # if recieved newer email --> update SQL entry
                   # Output for Error debug (root@kuvalda.lv)
#                    print
#                    print datetime.datetime.now()
#                    print mailbox_data
                   # Update SQL
#                    print '  Update MySQL entry'
                    update_sql_entry(d[4], temp - datetime.timedelta(hours=3))
                    d[1] = temp

            if mailbox_data[1] == True:
               # Update Images
#                print '  Update Images'
                time.sleep(4)
